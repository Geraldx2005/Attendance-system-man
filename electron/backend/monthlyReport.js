    ...
    for (let day = 1; day <= limitDay; day++) {
      ...
      if (log) {
        ...
        if (!isSunday && inMins !== null && expectedInMin !== null && Math.floor(inMins) > Math.floor(expectedInMin)) {
          stats.totalLateMins += Math.floor(inMins) - Math.floor(expectedInMin);
        }
...
/**
 * Convert HH:MM[:SS] to minutes since midnight (keeps second precision)
 */
